import {
  ArrowTrendingUpIcon,
  ClockIcon,
  FireIcon,
  UsersIcon,
} from "@heroicons/react/24/outline";
import React from "react";

function PublicSpeech() {
  return (
    <div className="bg-[#231246] border border-[#39246c] rounded-[16px] relative shadow-sm p-6">
      <div className="bg-[#8861ea] p-2 rounded-tr-[16px] rounded-bl-[16px] absolute right-0 top-0">
        <p className="text-xs font-bold text-white text-center">Featured</p>
      </div>

      <h5 className="text-xl font-bold text-white">Public Speech</h5>
      <h5 className="text-sm font-bold text-[#bebace] mt-2">
        Prepare and deliver a 3-minute speech on a topic of your choice.
      </h5>
      <div className="mt-4 flex items-center w-full">
        <div className="space-y-4 mt-4 w-full">
          <div className="flex items-center space-x-4">
            <div className="relative rounded-full px-3 font-bold py-1 text-[12px] text-[#ef4444] ring-[#482c85] ring-1 bg-[#341139]">
              Advanced
            </div>
            <div className="relative rounded-full font-bold px-3 py-1 text-[12px] text-[#8660e7] ring-1 ring-[#3b2372] bg-[#26164c]">
              500 Points
            </div>
            <div className="relative flex items-center font-bold justify-center gap-1 rounded-full px-3 py-1 text-[12px] text-[#d2cce8] ring-1 ring-[#4b2f8d] bg-[#4b2f8d]">
              <ClockIcon className="w-4 h-4 text-white" />
              22 days left
            </div>
          </div>
          <div className="flex items-center w-full">
            <p className="text-[#d1cde1] text-[12px] font-bold flex items-center justify-center gap-2">
              <UsersIcon className="w-5 h-5 text-white" />
              653 Participants
            </p>
            <p className="text-[#d1cde1] text-[12px] font-[600] ml-auto flex items-center justify-center gap-2">
              <ArrowTrendingUpIcon className="w-5 h-5 text-white " />
              Level 6
            </p>
          </div>
          <div className="w-full bg-[#4b2f8d] rounded-full h-1">
            <div
              className="bg-[#8861ea] h-1 rounded-tl-lg rounded-bl-lg"
              style={{ width: "0%" }}
            />
          </div>

          <button
            type="button"
            className="text-[#f1eefd] flex items-center justify-center gap-2 font-[600] cursor-pointer w-full mt-2 bg-[#8861ea] hover:bg-[#8861ea] focus:ring-4 focus:ring-[#8861ea]  rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none"
          >
            <FireIcon className="w-4 h-4 text-white" />
            Start Challenge
          </button>
        </div>
      </div>
    </div>
  );
}

export default PublicSpeech;
